...
return $selection;
  ...
  function addOtherOptions($plans)
  {
    ...
    //Minimum cart value MODULE_PAYMENT_FINANCEPAYMENT_MAX_CART
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Cart amount minimum', 'MODULE_PAYMENT_FINANCEPAYMENT_MAX_CART', '25000', 'Cart amount maximum for the Finance payment module', '6', '0', now())");
...
public function getProductPlans($product_price, $products_id)
{
  ...
  public function getCartPlans($order,$string = false)
  {
      ...
      if(MODULE_PAYMENT_FINANCEPAYMENT_WHOLE_CART == 'True' && (int)MODULE_PAYMENT_FINANCEPAYMENT_MIN_CART > $order->info['total'] && (int)MODULE_PAYMENT_FINANCEPAYMENT_MAX_CART < $order->info['total']) {
          return array();
      }
      if($string) {
        foreach ($plans as $key => $value) {
